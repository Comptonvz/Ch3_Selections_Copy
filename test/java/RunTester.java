...
{      
   ...
   {
      String path = tester.getPathToTestJava();
      ...
      try
      {
         ...
         String prevTestClass = testCases.get(0).getClassName();
         int j = 1;
         for( TestCase testCase : testCases )
         {
            if( !testCase.getClassName().equals( prevTestClass ) )
               j = 1;
            
            String testClassLabel = testCase.getClassName().replace(".java", "");
         
            outputWriter.write(String.format( "****%s - %d****%n", testClassLabel, j));
            outputWriter.write("\tClass: " + testCase.getClassName() + "\n");
            outputWriter.write("\tMethod: " + testCase.getMethodName() + "\n");
            outputWriter.write("\tInput(s): " + testCase.getMethodName() + "\n");
            outputWriter.write("\tPassed: " + testCase.passed());
            outputWriter.newLine();
            outputWriter.newLine();
            
            prevTestClass = testCase.getClassName();
            j++;
         }
         outputWriter.flush();
         outputWriter.close();
   ...
   public void createTestCaseButtons()
   {
      ...
      {
         if( !testCases.get(i).getClassName().equals( prevTestClass ) )
            j = 1;
            
         String testClassLabel = testCases.get(i).getClassName().replace(".java", "");
         ...
         prevTestClass = testCases.get(i).getClassName();
         j++;
      }
   }
   ...
   {
      classTitleLabel2.setText(""+testCase.getClassName());
      methodTitleLabel2.setText(""+testCase.getMethodName());
      paramTitleLabel2.setText(""+testCase.getMethodName());
      expectedText.setText( ""+testCase.getExpectedOutput() );
